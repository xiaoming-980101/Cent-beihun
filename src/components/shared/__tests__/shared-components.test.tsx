...
describe('Shared Components', () => {
  ...
  it('renders FeatureCard with required props', () => {
    render(
      <FeatureCard
        icon={<Package data-testid="feature-icon" className="h-5 w-5 text-purple-600" />}
        title="礼金簿"
    ...
    expect(screen.getByRole('button', { name: '进入工具：礼金簿' })).toBeInTheDocument();
  });
  ...
  it('triggers FeatureCard action when action button is clicked', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '查看详情：统计分析' }));
  ...
  it('renders EmptyState with optional description and action', () => {
    ...
    expect(screen.getByText('点击按钮开始添加第一条记录')).toBeInTheDocument();
